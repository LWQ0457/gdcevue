<template>
  <div id="app">
    ...
    <h2
      style="color: #00a1ff; border-bottom: 1px solid #1e95ce; text-align: left ;margin-top:20px"
    >
      挂牌信息
    </h2>
    <el-form
      ...
      <el-row type="flex" justify="center">
        ...
        <el-col :span="6">
          <el-form-item label="订单金额:" prop="money">
            <el-input v-model="order.money" size="small" class="depositPrice" placeholder="(元)" disabled="disabled" />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row type="flex" justify="center">
        ...
        <el-col :span="6">
          <el-form-item label="煤种：" prop="coalType">
            <el-select v-model="coal.coalType" prop="type" class="type" placeholder="请选择煤种" size="small">
...
<script>
import { getAxios, postAxios } from '../api/request'
...
export default {
  ...
  watch: {
    ...
    'coal.tonnage': {
      handler: function() {
        this.order.money = this.coal.tonnage * this.coal.unitPrice
      }
    },
    'coal.unitPrice': {
      handler: function() {
  ...
  created() {
    if (this.$route.params.order != null) {
      ...
      const params = { 'orderNum': this.order.num }
  ...
  methods: {
    ...
    submit() {
      const params = { 'order': this.order, 'coal': this.coal }
      let valid1 = false
      let valid2 = false
      ...
      if (valid1 && valid2) {
        postAxios('trade/modify', params).then(res => {

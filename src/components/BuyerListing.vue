    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.state=='等待摘牌'&&user.userType==1" type="text" size="small" @click="handleAcceptClick(scope.row)">摘牌</el-button>
        </template>
...
export default {
  ...
  methods: {
      ...
      if (row != null) {
        const data = {
          ...
          'params': {
            ...
            'allow': (row.row.state == '等待摘牌' && this.user.userType == 1)
    ...
    pageChange: function(currentPage) {
      ...
      this.tableData = []
      console.log(params)
      getAxios('/trade/buyer/list', params).then(res => {
        console.log(res)
      ...
      for (var i = 0; i < this.tableData.length; i++) {
        ...
        getAxios('/trade/coal/query', params1).then(res => {
          console.log(res)
          this.coal.push(res.data)// 不严谨，明天再换
          this.loading = false
    ...
    accept(orderNum) {
      ...
      getAxios('/trade/seller/accept', params).then(res => {
